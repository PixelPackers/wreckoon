...
public class SmartPig extends Enemy {
	...
	public SmartPig(Game game, float posX, float posY, float width, float height, float density, float friction, float restitution,
	...
	public void die() {
		super.die();
		if(!aggro && game.getPlayer().isAttacking()){
	...
	public void update() {
		...
		if (!player.isDead() && ((distance < AGGRO_DISTANCE && aggro) || (distance < AGGRO_DISTANCE && playerIsLeft() == left))) {
			aggro = true;
		} else {
			if (aggro) {
				aggro = false;
...
//						 && player.isLookingLeft() != (x<0) // nur wenn spieler zum schwein schaut
				...
				if (this.isOnGround()) {
					...
					// this.getBody().applyLinearImpulse( new Vec2(x,
					// 0*this.getBody().getLinearVelocity().y),
